...
class AsciiFilter {
  ...
  constructor(renderer: THREE.WebGLRenderer, { fontSize, fontFamily, charset, invert }: AsciiFilterOptions = {}) {
    ...
    (this.context as CanvasRenderingContext2D & { webkitImageSmoothingEnabled?: boolean; mozImageSmoothingEnabled?: boolean; msImageSmoothingEnabled?: boolean }).webkitImageSmoothingEnabled = false;
    (this.context as CanvasRenderingContext2D & { mozImageSmoothingEnabled?: boolean }).mozImageSmoothingEnabled = false;
    (this.context as CanvasRenderingContext2D & { msImageSmoothingEnabled?: boolean }).msImageSmoothingEnabled = false;
    ...
    this.scene.traverse(obj => {
      if ((obj as THREE.Mesh).isMesh) {
        ...
        if (mesh.material && typeof mesh.material === 'object') {
          const mat = mesh.material as THREE.Material & Record<string, unknown>;
          Object.keys(mat).forEach(key => {
            const matProp = mat[key];
            if (matProp !== null && typeof matProp === 'object' && typeof (matProp as { dispose?: () => void }).dispose === 'function') {
